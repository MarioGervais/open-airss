...
strokeBezier(s(4), s(13.5), s(8.14), s(13.5), s(11.5), s(16.86), s(11.5), s(21), s(2.5));

// Outer arc: M4 7.5 C10.5 5.5 18 9.5 20.5 17
strokeBezier(s(4), s(7.5), s(10.5), s(5.5), s(18), s(9.5), s(20.5), s(17), s(2));
...
buf.writeUInt16LE(1, off); off += 2;      // image count
...
buf.writeUInt16LE(32, off); off += 2;     // bit depth
buf.writeUInt32LE(bmpBytes, off); off += 4; // size of BMP block
buf.writeUInt32LE(22, off); off += 4;     // offset to BMP block (6+16=22)

// BITMAPINFOHEADER
buf.writeUInt32LE(40,        off); off += 4; // biSize
buf.writeInt32LE( SIZE,      off); off += 4; // biWidth
buf.writeInt32LE( SIZE * 2,  off); off += 4; // biHeight (×2 includes AND mask)
